...
window.sfBlazor.PivotView = function () {
  ...
  function () {
    ...
    PivotButton.prototype.setPivotButtonDrag = function () {
      for (var _i = 0, _a = [].slice.call(this.parent.parentElement.querySelectorAll('.' + (this.parent instanceof SfPivotView ? GROUPING_BAR_CLASS : PIVOT_FIELD_LIST_CLASS) + '-button')); _i < _a.length; _i++) {
        ...
        this.parent.pivotButtonModule.createDraggable(false, this.parent instanceof SfPivotView ? buttonElement.querySelector('.' + PIVOT_BUTTON_CONTENT_CLASS) : buttonElement.firstElementChild, JSON.parse(buttonElement.getAttribute('data-fieldInfo')));
      }
    };
    ...
    PivotButton.prototype.onDragStop = function (isTreeElement, args) {
      ...
      if (!isTreeElement && args.target.classList && (args.target.classList.contains(GROUP_CHART_VALUE) || args.target.classList.contains(GROUP_CHART_VALUE_DROPDOWN))) {
        args.target = this.parent.element.querySelector('.' + GROUP_CHART_ROW);
      }

      if (!isTreeElement && args.target.classList && args.element && (args.target.classList.contains(GROUP_CHART_COLUMN) || args.target.classList.contains(GROUP_CHART_COLUMN_DROPDOWN))) {
        cancel = true;
      }
  ...
  function () {
    ...
    CalculatedField.prototype.editCalculatedFieldInfo = function (isEdit, top, left, title) {
      ...
      if (isEdit) {
        ...
        document.querySelector('#' + this.parent.element.id + 'droppable').value = treeNode.getAttribute('data-uid');
      } else {
  ...
  function () {
    ...
    DrillThrough.prototype.mouseClickHandler = function (e) {
      ...
      if (target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(CELLVALUE) || target.classList.contains(HEADER_CELL_DIV)) {
  ...
  function () {
    ...
    ActionBase.prototype.validateInputs = function (filterInfo) {
      ...
      if (sf.base.isNullOrUndefined(operand1) || operand1 === '' || ['Between', 'NotBetween'].indexOf(operator) > -1 && (sf.base.isNullOrUndefined(operand2) || operand2 === '')) {
        var focusElement = this.parent.parentElement.querySelector('#' + this.parent.element.id + '_' + type.toLowerCase() + (sf.base.isNullOrUndefined(operand1) || operand1 === '' ? '_input_option_1' : '_input_option_2'));
  ...
  function () {
    ...
    KeyboardInteraction.prototype.getAdjacentButton = function (target, position) {
      ...
      if (this.parent.internalGrid && this.parent.internalGrid.element.querySelector('.' + PIVOT_BUTTON_CLASS)) {
  ...
  function () {
    ...
    VirtualScroll.prototype.update = function (mHdr, mCont, top, left, e) {
      ...
      if (this.parent.pageSettings) {
        if (this.direction === 'vertical') {
          ...
          var exactSize = this.parent.pageSettings.columnSize * colValues * this.parent.gridSettings.columnWidth;
    ...
    VirtualScroll.prototype.onHorizondalScroll = function (mHdr, mCont, fCont) {
      ...
      return function (e) {
        ...
        if (_this.parent.scrollPosObject.horizontalSection > left ? true : excessMove > 1 && notLastPage) {
          ...
          var vWidth = _this.parent.gridSettings.columnWidth * _this.parent.scrollPageInfo.columnCount - _this.parent.internalGrid.columnModel[0].width;
    ...
    VirtualScroll.prototype.onVerticalScroll = function (fCont, mCont) {
      ...
      return function (e) {
        ...
        var excessMove = _this.parent.scrollPosObject.verticalSection > top ? -(_this.parent.scrollPosObject.verticalSection - top) : top + fCont.clientHeight - (_this.parent.scrollPosObject.verticalSection + fCont.querySelector('.' + TABLE).offsetHeight);
    ...
    VirtualScroll.prototype.updateScrollInfo = function (element, columnCount, rowCount) {
      if (this.parent.options.enableVirtualization) {
        ...
        if (element.querySelector('.' + MOVABLECONTENT_DIV) && !element.querySelector('.' + MOVABLECONTENT_DIV).querySelector('.' + VIRTUALTRACK_DIV)) {
          virtualDiv = sf.base.createElement('div', {
        ...
        if (element.querySelector('.' + MOVABLEHEADER_DIV) && !element.querySelector('.' + MOVABLEHEADER_DIV).querySelector('.' + VIRTUALTRACK_DIV)) {
          virtualHeaderDiv = sf.base.createElement('div', {
            className: VIRTUALTRACK_DIV
          });
        ...
        var vWidth = this.parent.gridSettings.columnWidth * columnCount - this.parent.internalGrid.columnModel[0].width;
        ...
        var verOffset = mCnt.scrollTop > this.parent.scrollerBrowserLimit ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[1].trim() : -(mCnt.scrollTop * this.parent.verticalScrollScale - this.parent.scrollPosObject.verticalSection - mCnt.scrollTop) + 'px)';
        var horiOffset = mCnt.scrollLeft > this.parent.scrollerBrowserLimit ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[0].trim() + ',' : 'translate(' + -(mCnt.scrollLeft * this.parent.horizontalScrollScale - this.parent.scrollPosObject.horizontalSection - mCnt.scrollLeft) + 'px,';
    ...
    SfPivotView.prototype.mouseClickHandler = function (e) {
      ...
      if (this.enableValueSorting && this.options.dataType === 'pivot' && (target.classList.contains(SORT_FILTER_DIV) || target.classList.contains(STACKED_HEADER_TEXT) || target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(HEADER_TEXT) || target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(HEADER_CELL_DIV) || target.classList.contains(HEADERCELL) || target.classList.contains(CELLVALUE) || target.classList.contains(COLUMNSHEADER) || target.classList.contains(ROWSHEADER))) {
        ...
        if (element !== null) {
          var colIndex = Number(element.getAttribute('aria-colindex'));
          var rowIndex = Number(element.getAttribute('index'));
          if (!(target.classList.contains('e-expand') || target.classList.contains('e-collapse'))) this.dotNetRef.invokeMethodAsync('CellClickedHandler', rowIndex, colIndex, e, JSON.stringify(window.sfBlazor.getDomObject('cellElement', element)));
        }
      }
    };
    /* eslint-enable */
  ...
  function () {
    ...
    SfPivotFieldList.prototype.getOptions = function (element, options) {
      ...
      if (this.parentElement && this.parentElement.querySelector('#' + this.parentElement.id + '_title')) {
        sf.base.setStyleAttribute(this.parentElement.querySelector('#' + this.parentElement.id + '_title'), {
          'width': '100%'
        });
      }
    };
  ...
  var PivotView = {
    ...
    updateGridUI: function updateGridUI(element) {
      if (element && element.blazor__instance) {
