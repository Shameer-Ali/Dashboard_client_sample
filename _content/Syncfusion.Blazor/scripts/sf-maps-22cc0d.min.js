...
window.sfBlazor.Maps = function () {
  ...
  function calculateShape(shape, size, location) {
    ...
    switch (shape) {
      ...
      case 'Pentagon':
        ...
        var factor = 72;
        var xValue = void 0;
        var yValue = void 0; // eslint-disable-next-line no-case-declarations
  ...
  function () {
    ...
    function SfMaps(id, element, options, dotnetRef) {
      ...
      this.lastScale = 1;
      this.isTouch = false; // eslint-disable-next-line @typescript-eslint/no-explicit-any

      this.touchMoveList = []; // eslint-disable-next-line @typescript-eslint/no-explicit-any

      this.touchStartList = []; // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      this.options.markerSettings = JSON.parse(this.options.markerSettings);
    ...
    SfMaps.prototype.rightClick = function (event) {
      var id = event.target['id']; // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    SfMaps.prototype.markerTranslate = function (element, factor, x, y, scale, template) {
      ...
      if (latitude && longitude) {
        if (this.isTileMap) {
          ...
          point['x'] = point['x'] + this.options.markerSettings[layerIndex][markerIndex]['X'];
          point['y'] = point['y'] + this.options.markerSettings[layerIndex][markerIndex]['Y'];
          ...
          location_2.x = (location_2.x + x) * scale + this.options.markerSettings[layerIndex][markerIndex]['X'];
          location_2.y = (location_2.y + y) * scale + this.options.markerSettings[layerIndex][markerIndex]['Y'];
    ...
    SfMaps.prototype.dataLabelTranslate = function (element, factor, x, y, scale, type) {
      ...
      if (type !== 'Template') {
        ...
        if (element.id.indexOf('_RectIndex_') > -1) {
          ...
          element.setAttribute('transform', 'translate( ' + labelX + ' ' + (labelY + padding) + ' )');
        }
      } else {
    ...
    SfMaps.prototype.convertGeoToPoint = function (latitude, longitude, factor) {
      ...
      var size = factor === null || factor === undefined ? Math.min(mapSize.width, mapSize.height) : Math.min(mapSize.width, mapSize.height) * factor; // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      switch (type) {
        case 'Mercator':
          ...
          x = pixelOrigin.x + longitude * (size / 360); // eslint-disable-next-line no-case-declarations
        ...
        case 'Eckert6':
          // eslint-disable-next-line no-case-declarations
          var epsilon = 1e-6;
          temp = (1 + Math.PI / 2) * Math.sin(latRadian); // eslint-disable-next-line no-case-declarations

          var delta = Infinity; // eslint-disable-next-line no-case-declarations
    ...
    SfMaps.prototype.clusterExpand = function (target, options, dotnetRef) {
      ...
      var layerIndex = parseInt(target[0].split('_LayerIndex_')[1].split('_')[0], 10); // eslint-disable-next-line @typescript-eslint/no-explicit-any

      var clusterSetting = this.markerClusterCollection[layerIndex]; // eslint-disable-next-line @typescript-eslint/no-explicit-any

      var data;
      var id = target[0].split('_LayerIndex_');
      var index = parseInt(id[1].split('_')[0], 10); // eslint-disable-next-line @typescript-eslint/no-explicit-any

      var markCollection = []; // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (target[0].indexOf('_MarkerIndex_') > -1) {
        ...
        if (!isNaN(markerIndex)) {
          data = datasource[dataIndex]; // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    SfMaps.prototype.clusterSeparate = function (sameMarkerData, markerElement, isDom, id, // eslint-disable-next-line @typescript-eslint/no-explicit-any
    connectorLine) {
      var layerIndex = sameMarkerData[0].layerIndex;
      var markerIndex = sameMarkerData[0].markerIndex;
      var clusterIndex = sameMarkerData[0].targetClusterIndex;
    ...
    SfMaps.prototype.clusterMarkers = function (layerIndex) {
      ...
      var postionY = 15 / 4;
      var m = 0; // eslint-disable-next-line @typescript-eslint/no-explicit-any

      var markerCollection;
      var tempElement; // eslint-disable-next-line @typescript-eslint/no-explicit-any

      var balloonGroup; // eslint-disable-next-line @typescript-eslint/no-explicit-any

      var clusterColloideBounds = [];
      var markerTemplateCheck;
          ...
          if (markerTemplate.children[o]['style']['visibility'] !== 'hidden' && markerIndex === 0) {
            ...
            if (bounds1_1 !== null) {
              ...
              if (colloideBounds.length > 0) {
                ...
                m++; // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                switch (markerCluster.Shape) {
                  ...
                  case 'Balloon':
                    // eslint-disable-next-line no-case-declarations
                    balloonGroup = createGroup(clusterID); // eslint-disable-next-line no-case-declarations
                  ...
                  case 'Image':
                    // eslint-disable-next-line no-case-declarations
                    var locationX = 0; // eslint-disable-next-line no-case-declarations

                    var locationY = 0; // eslint-disable-next-line no-case-declarations

                    var x = -(locationX + markerCluster.Width / 2); // eslint-disable-next-line no-case-declarations
                ...
                shape.setAttribute('class', 'clusterGroup'); // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    SfMaps.prototype.createClusterShape = function (shapeType, id, fill, opacity, // eslint-disable-next-line @typescript-eslint/no-explicit-any
    transform, isClass, indexCollection, border) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    SfMaps.prototype.imageExport = function (type, fileName, allowDownload) {
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
          proxy = this; // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          , new Promise(function (resolve, reject) {
            ...
            proxy.exportedCount = 0; // eslint-disable-next-line @typescript-eslint/no-explicit-any

            var promises = [];

            for (var i = 0; i < tileElementCount; i++) {
              var tile = document.getElementById(proxy.element.id + 'tile' + i); // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    SfMaps.prototype.renderImages = function (canvasElement, tile, context, exportTileImg, tileElementCount, // eslint-disable-next-line @typescript-eslint/no-explicit-any
    type, fileName, url, proxy, allowDownload) {
  ...
  var Maps = {
    ...
    markerCluster: function markerCluster(element) {
      if (element && element.blazor__instance) {
        ...
        for (var i = 0; i < element.blazor__instance.layerCount; i++) {
          // eslint-disable-next-line curly
          if (element.blazor__instance.markerClusterCollection[i].AllowClustering) element.blazor__instance.clusterMarkers(i);
        }
      }
    },
    ...
    getToolbarAlign: function getToolbarAlign(id, data, element) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      var verticalAlignment = data.verticalAlignment; // eslint-disable-next-line @typescript-eslint/no-explicit-any
